...
def has_attachment(comment_id: str) -> bool:
    with get_connection() as connection:
        row = connection.execute(
            "SELECT 1 FROM comment_attachments WHERE comment_id = ? LIMIT 1",
            (comment_id,),
        ).fetchone()
...
def parse_video_analysis_content(content: str) -> dict[str, Any] | None:
    """Parse video_analysis content JSON, return dict with location/behavior/environment or None.

    Shared utility — callers format the result however they need:
    - Chinese display string for AI context
    - JSON dict for template rendering
    - Multiline string for push API
    """
    if not content:
        return None
    try:
